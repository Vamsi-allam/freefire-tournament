...
import { createMatch, listUpcomingMatches, listMatches, updateMatch, deleteMatch, saveCredentials, sendCredentialsToPlayers, getMatchRegistrations } from '../utils/api';
...
const AdminPage = () => {
  ...
  const syncType = (title) => {
    ...
    if (lower.includes('clash') || lower.includes('4v4') || lower.includes('4 vs 4')) return 'CLASH_SQUAD';
  ...
  // Light auto-refresh to reflect status changes (e.g., auto-cancel) without manual reload
  useEffect(() => {
    const tabsNeedingRefresh = ['scheduler', 'manage', 'history'];
    if (!tabsNeedingRefresh.includes(activeTab)) return;
  ...
  const computed = (() => {
    ...
    const slots = type === 'SOLO' ? 48 : type === 'DUO' ? 24 : (type === 'SQUAD' ? 12 : 2);
    const pool = type === 'CLASH_SQUAD' ? Math.round(form.entryFee * slots * 0.85) : (form.entryFee * slots);
    return {
      slots,
      pool,
      first: type === 'CLASH_SQUAD' ? pool : Math.round(pool * 0.40),
  ...
  const submit = async (e) => {
    ...
    try {
  const payload = { ...form };
  if (payload.matchType !== 'CLASH_SQUAD') delete payload.rounds; else payload.rounds = Number(payload.rounds) || 7;
  ...
  setEditingMatch({...match, rounds: Number(match.rounds) || 7});
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      if (String(payload.matchType).toUpperCase() === 'CLASH_SQUAD') {
        payload.slots = 2;
        payload.rounds = Number(payload.rounds) || 7;
      } else {
  ...
  const response = await fetch(`${API_BASE}/api/upi/admin/pending`, {
      ...
      if (response.ok) {
        ...
        console.error('Failed to load UPI payments');
        setUpiPayments([]);
      }
    } catch (error) {
      console.error('Error loading UPI payments:', error);
      setUpiPayments([]);
    } finally {
  ...
  const response = await fetch(`${API_BASE}/api/upi/admin/action`, {
      ...
      if (response.ok) {
        openSnack(data.message || `Payment ${action.toLowerCase()}d successfully!`, 'success');
        loadUpiPayments(); // Refresh the list
      } else {
        openSnack(data.error || `Failed to ${action.toLowerCase()} payment`, 'error');
      }
    } catch (error) {
      console.error(`Error ${action.toLowerCase()}ing payment:`, error);
      openSnack(`Failed to ${action.toLowerCase()} payment. Please try again.`, 'error');
    } finally {
                    ...
                    <div key={m.id} className="ap-match-card">
                      <div className="ap-match-header">
                        <div className="ap-match-title-section">
                          ...
                          <div className="ap-match-date">{new Date(m.scheduledAt).toLocaleString()} • {m.mapName || m.map || 'Map N/A'}</div>
                        </div>
          ...
          <option>Free Fire Clash Squad (4v4)</option>
                ...
                {form.matchType === 'CLASH_SQUAD' && (
                  <label className="ap-field">Rounds
                    <select name="rounds" value={form.rounds} onChange={(e)=> setForm(f=>({...f, rounds: Number(e.target.value)||7}))}>
              ...
              <div className="ap-prizes">
                ...
                <div>Total Pool: <strong>₹{computed.pool}</strong>{form.matchType==='CLASH_SQUAD' && <span style={{marginLeft:6, opacity:.75}}>(85% winner takes all)</span>}</div>
                    ...
                    <div key={payment.id} className="ap-upi-payment-card">
                      <div className="ap-payment-header">
                        ...
                        <div className="ap-payment-status">
                          ...
                          <div className="ap-payment-date">{new Date(payment.createdAt).toLocaleString()}</div>
                        </div>
                      </div>
                      ...
                      <div className="ap-payment-details">
                        ...
                        <div className="ap-detail-row">
                          <span className="ap-detail-label">Payee UPI:</span>
                          <span className="ap-detail-value">{payment.payeeVpa}</span>
                        </div>
                        <div className="ap-detail-row">
                          <span className="ap-detail-label">Payee Name:</span>
                          <span className="ap-detail-value">{payment.payeeName}</span>
                        </div>
                        <div className="ap-detail-row">
                        ...
                        <div className="ap-detail-row">
                          <span className="ap-detail-label">Payment App:</span>
                          <span className="ap-detail-value">{payment.paymentApp}</span>
                        </div>
                      </div>
              ...
              <div className="ap-modal-form">
                ...
                <div className="ap-form-row">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setEditingMatch(prev => ({
                          ...
                          matchType: mt,
                          gameMode: mt,
                          slots: mt === 'SOLO' ? 48 : mt === 'DUO' ? 24 : (mt === 'SQUAD' ? 12 : 2),
                          rounds: mt === 'CLASH_SQUAD' ? (Number(prev.rounds) || 7) : undefined
                        }));
                      }}
                  ...
                  <div className="ap-form-group">
                    ...
                    <input
                      type="number"
                      value={String(editingMatch.matchType).toUpperCase()==='CLASH_SQUAD' ? 2 : editingMatch.slots}
                      onChange={(e) => setEditingMatch({...editingMatch, slots: parseInt(e.target.value)})}
                      readOnly={String(editingMatch.matchType).toUpperCase()==='CLASH_SQUAD'}
                      className="ap-form-input"
                    />
                  </div>
                </div>
                ...
                {String(editingMatch.matchType).toUpperCase()==='CLASH_SQUAD' && (
                  <div className="ap-form-group">
                    <label>Rounds</label>
                    <select
                      value={Number(editingMatch.rounds) || 7}
                      onChange={(e) => setEditingMatch({...editingMatch, rounds: Number(e.target.value) || 7})}
                      className="ap-form-select"
                    >
                      <option value={7}>Best of 7</option>
                      <option value={13}>Best of 13</option>
                  ...
                  <div key={reg.id} className="ap-player-slot">
                    ...
                    {reg.players && (
                      <ul className="ap-slot-roster">
                        {reg.players.sort((p1,p2)=>p1.position-p2.position).map(p => (
                          <li key={p.gameId || p.gameName} className="ap-slot-player-line">
