...
const SupportModal = ({ isOpen, onClose, defaultEmail = '', defaultPhone = '' }) => {
  ...
  await submitSupportRequest({ ...form });
      ...
      alert('Thanks! Your support request has been submitted. We\'ll get back to you soon.');
      onClose?.();
    } catch (err) {
      console.error('Support submit failed:', err);
      alert(err?.message || 'Failed to submit support request. Please try again later.');
    } finally {
