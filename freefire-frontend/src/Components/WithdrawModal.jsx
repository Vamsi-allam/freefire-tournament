...
const WithdrawModal = ({ isOpen, onClose, onWithdraw, currentBalance }) => {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      // Store withdrawal data for OTP modal
  ...
  const handleOtpSuccess = async (transactionResult) => {
    try {
      // Call parent's onWithdraw with the completed transaction
      await onWithdraw(transactionResult);
      ...
      setError('Withdrawal completed but failed to update UI. Please refresh the page.');
    }
  };
