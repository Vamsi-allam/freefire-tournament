...
const UserPage = () => {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        if (data.user) {
          // Update Redux store with latest user data including avatar
          dispatch(updateProfile({
            displayName: data.user.name,
            phoneNumber: data.user.phone,
            gameId: data.user.gameId,
  ...
  // Auto-refresh every 15 seconds to keep data (matches, registrations, wallet, profile) in sync with admin changes
  useEffect(() => {
  ...
  const checkAuthStatus = () => {
    ...
    if (token) {
      ...
      .catch(err => console.log("Auth debug - Wallet API error:", err));
    }
  };
  ...
  const handleSaveProfile = async (profileData) => {
    try {
      ...
      if (response.ok) {
        ...
        // Update Redux store with the updated user data from backend response
                      ...
                      <div className="card-header">
                        <div className="card-badges">
                          ...
                          {matchWithStatus.canViewRoomCredentials && (
                            <span className="status-badge room-ready">
                              🎮 ROOM READY
                            </span>
                          )}
                        </div>
